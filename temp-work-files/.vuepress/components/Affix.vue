...
export default {
  ...
  watch: {
    ...
    slotComponentIsMounted: {
      immediate: true,
      handler(val) {
        console.log('slotComponentIsMounted', val)
        if (val) {
          this.init();
        }
      }
  ...
  methods: {
    init() {
      ...
      console.log('affixInitialTop', this.affixInitialTop)
      console.log('this.getOffsetTop(this.relativeElement)', this.getOffsetTop(this.relativeElement))
